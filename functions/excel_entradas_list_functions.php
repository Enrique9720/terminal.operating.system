<?php
function excel_listado_entradas($entradas_list)
{
  ...
  $nombre_comercial_propietario = 'CCIS-BILBAO_SICSA-VALENCIA';*/
    $filename = "../excel/listados/entradas/LISTADO ENTRADAS.xlsx";
    $spreadsheet = new PhpOffice\PhpSpreadsheet\Spreadsheet();
    $activeWorksheet = $spreadsheet->getActiveSheet();

    //Nombre de la hoja activa
    $activeWorksheet->setTitle("Entradas");

    //Texto para titulo A1
    $activeWorksheet->setCellValue('A1', 'LISTADO ENTRADAS');
    ...
    $activeWorksheet->getStyle('F3')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('92D050');
    $activeWorksheet->getStyle('G3')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('92D050');

    //Texto para encabezado A2
    $activeWorksheet->setCellValue('A2', 'Nº EXPEDICION');
    //Texto para encabezado B2
    $activeWorksheet->setCellValue('B2', 'FECHA ENTRADA');
  ...
  $activeWorksheet->getStyle('F')->getNumberFormat()->setFormatCode(PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_DATE_DDMMYYYY);*/
    ...
    foreach ($entradas_list as $key => $entradas_line) {
        ...
        $activeWorksheet->setCellValue('A' . $indice, $entradas_line['num_expedicion']);
        //Valor para celda B.$indice
        $activeWorksheet->setCellValue('B' . $indice, $entradas_line['fecha_entrada']);
        //Valor para celda C.$indice
        $activeWorksheet->setCellValue('C' . $indice, $entradas_line['tipo_entrada']);
        //Valor para celda D.$indice
        $activeWorksheet->setCellValue('D' . $indice, $entradas_line['nombre_comercial_propietario']);
        //Valor para celda D.$indice
        $activeWorksheet->setCellValue('E' . $indice, $entradas_line['contenedores'][0]['total_contenedores']);

        foreach ($entradas_line['contenedores'] as $key => $contenedor) {
