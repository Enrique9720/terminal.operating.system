...
$list = $railsider_model -> get_estaciones_ferrocarril_renfe_ajax($search);

// Nos aseguramos que hallan resultados
if(count($list) > 0){
	foreach ($list as $key => $value) {
		$data[] = array(
			'id' => $value['codigo_estacion_ferrocarril'],
			'text' => $value['nombre_estacion_ferrocarril']." (".$value['codigo_estacion_ferrocarril'].")"
		);
	}
} else {
	$data[] = array(
